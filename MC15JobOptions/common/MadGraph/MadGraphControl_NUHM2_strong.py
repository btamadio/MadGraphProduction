...
fcard.write("""
    ...
    generate p p > accessiblestrong accessiblestrong QED=99 QCD=99 @1
    add process p p > accessiblestrong accessiblestrong j QED=99 QCD=99 @2
    output -f
    """)
fcard.close()
...
runArgs.qcut = 200. # default MLM matching scale
...
evgenConfig.description = "NUHM2 test model with strong production, m_12 = %s GeV"%(runArgs.jobConfig[0].split('_')[4])
#evgenConfig.inputfilecheck = runArgs.inputGeneratorFile.split('._0')[0]
evgenConfig.keywords = ["SUSY","NUHM2"]
evgenConfig.process = "gg"
