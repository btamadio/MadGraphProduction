...
from MadGraphControl_RPVSimplifiedModelUtils import SUSY_SM_Generation
...
if not SLHAonly:
    [qcut,outputDS] = SUSY_SM_Generation(runArgs=runArgs,process=process,gentype=gentype,decaytype=decaytype,masses=masses,nevts=nevts,syst_mod=syst_mod, writeGridpack=writeGridpack, gridpackDirName=gridpackDirName, keepOutput=keepOutput, pdlabel="'nn23lo1'",decays=decays)
else:
    [qcut,outputDS] = SUSY_SM_Generation(runArgs=runArgs,process=process,gentype=gentype,decaytype=decaytype,masses=masses,nevts=10000,syst_mod=None, SLHAonly=True, keepOutput=keepOutput,decays=decays)
    modify_run_card('run_card.dat','run_card_backup.dat',settings={'bwcutoff':2})
