...
Wxi = 0.0
if Mxi > 2*Mchi:
  Wxi = Wxi + gxichi*gxichi/12./math.pi/Mxi * (Mxi*Mxi + 2*Mchi*Mchi) * math.sqrt(1 - 4*Mchi*Mchi/Mxi/Mxi)
for mq in mquark:
  if Mxi > 2*mq:
    Wxi = Wxi + 3*(gxiU*gxiU/12./math.pi/Mxi * (Mxi*Mxi + 2*mq*mq) * math.sqrt(1 - 4*mq*mq/Mxi/Mxi))
...
paramcard = subprocess.Popen(['get_files','-data','MadGraph_param_card_dmV_Wlv.dat'])
paramcard.wait()
if not os.access('MadGraph_param_card_dmV_Wlv.dat',os.R_OK):
    print 'ERROR: Could not get param card'
elif os.access('param_card.dat',os.R_OK):
    print 'ERROR: Old param card in the current directory.  Dont want to clobber it.  Please move it first.'
else:
    oldcard = open('MadGraph_param_card_dmV_Wlv.dat','r')
...
#### Shower                                                                                                                                                             
evgenConfig.description = "Wimp pair monoW with dmV"
