...
paramcard = subprocess.check_call(['get_files','-data',
                                   'MadGraph_param_card_dmS_WZhad.dat'])
if not os.access('MadGraph_param_card_dmS_WZhad.dat', os.R_OK):
    print 'ERROR: Could not get param card'
elif os.access('param_card.dat', os.R_OK):
    print ('ERROR: Old param card in the current directory.  Dont wan\'t to' 
           'clobber it.  Please move it first.')
else:
    oldcard = open('MadGraph_param_card_dmS_WZhad.dat', 'r')
...
evgenConfig.description = ('Wimp pair mono-WZ with simplified model, dmS, '
